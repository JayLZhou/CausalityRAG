"""Internal Transformer-message extraction for Contribution Graph construction."""
...
class _TransformerRuntime:
    """Shared model, tokenization, and message-attribution runtime."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or self.max_receivers_per_layer <= 0
        ):
            raise ValueError(
                "max_context_tokens and max_length must be non-negative; "
                "max_receivers_per_layer must be positive"
            )
...
class _MessageTrace:
    """Forward-pass tensors needed for message attribution."""
...
class _MessageTraceRuntime(_TransformerRuntime):
    """Capture the component writes required by contribution-flow attribution."""

    def _install_trace_hooks(self) -> _MessageTrace:
        trace = _MessageTrace()
...
class _ClosedMessageFlowExtractor(_MessageTraceRuntime):
    ...
    def extract(
        ...
        *,
        ...
        nodes = self._contribution_nodes(
        ...
        return {
            ...
            "target_answer": target_answer,
            "status": graph_status,
            "method": f"{self.method}_{self.target_objective}_closed_flow",
            ...
            "graph": {
                ...
                "positive_edge_policy": "retain_all_for_active_receivers",
                ...
                "signed_weight_field": "raw_signed_contribution",
                ...
                "flow_construction": (
                    "closed_backward_receiver_beam_all_positive_incoming_with_background"
                ),
    ...
    def _closed_flow_edges(
        ...
        for layer_index in range(layer_count - 1, -1, -1):
            ...
            if not active_mid:
                active_post = {}
                continue
            attention_matrix, _ = self._attention_message_scores(
                layer_index,
                position_embeddings,
