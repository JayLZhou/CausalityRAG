...
def test_context_units_check_context_hashes():
    record = {
        "id": "q1",
    ...
    assert units_from_context_row(record, row, k=1) == [{"unit_id": "u1"}]

    row["context_sha256"]["c1"] = "bad"
    try:
        units_from_context_row(record, row, k=1)
    except ValueError as error:
        assert "context hash mismatch" in str(error)
    else:
        raise AssertionError("stale context units must be rejected")
