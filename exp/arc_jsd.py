...
def solve_arc_jsd_ratio(
    units: list[dict],
    node_weights: dict[str, float],
    interactions: dict[tuple[str, str], float],
    _target_edges: dict[str, float] | None = None,
    *,
    max_iterations: int = 100,
    tolerance: float = 1e-9,
) -> dict:
    """Exactly solve the ARC-JSD token-lift density baseline.

    The baseline maximizes positive singleton and induced pair mass per
    selected token. Dinkelbach iterations use a maximum-closure min-cut oracle.
    It is intentionally isolated here rather than reused by ReFlow.
    """

    unit_ids = {str(unit["unit_id"]) for unit in units}
    weights = {
        unit_id: max(0.0, float(node_weights.get(unit_id, 0.0)))
        for unit_id in unit_ids
    }
    edges: dict[tuple[str, str], float] = defaultdict(float)
    for (left, right), weight in interactions.items():
        left, right = str(left), str(right)
        if left in unit_ids and right in unit_ids and left != right and float(weight) > 0.0:
            edges[tuple(sorted((left, right)))] += float(weight)
    weighted_edges = [(left, right, weight) for (left, right), weight in edges.items()]
    if not unit_ids or (not any(weights.values()) and not weighted_edges):
        return {"status": "no_positive_support", "selected_ids": [], "density": 0.0, "iterations": 0}

    def score(selected: set[str]) -> tuple[float, float]:
        unary = sum(weights[unit_id] for unit_id in selected)
        pairs = sum(weight for left, right, weight in weighted_edges if left in selected and right in selected)
        return unary, pairs
    ...
    for iteration in range(1, max_iterations + 1):
        selected, value = _maximum_weight_closure(weights, weighted_edges, density)
        if not selected or value <= tolerance:
            break
        unary, pairs = score(selected)
        next_density = (unary + pairs) / len(selected)
        history.append({"iteration": iteration, "density": next_density, "surrogate_value": value})
        best = selected
    ...
    unary, pairs = score(best)
    return {
        "status": "optimal" if best else "no_positive_subgraph",
        "selected_ids": sorted(best),
        "density": (unary + pairs) / len(best) if best else 0.0,
        "source_weight": unary,
        "target_weight": 0.0,
        "internal_weight": pairs,
...
def _maximum_weight_closure(
    ...
    return ({unit_id for unit_id, node in vertices.items() if node in reachable}, positive_profit - flow)
