"""MIRAGE context-token saliency over frozen reader trajectories."""
...
class MirageScorer:
    """Compute MIRAGE CTI followed by contrastive input-gradient saliency."""
    ...
    def score(
        ...
        contextless_log_probs = self.model._log_probs_for_prompt_ids(
            [contextless_prompt_ids],
            trajectory.response_ids,
        )[0]
        cti = (
            full_log_probs.exp() * (full_log_probs - contextless_log_probs)
        ).sum(dim=-1)
        selected, threshold = context_sensitive_indices(
            cti.float().cpu().tolist(),
            self.cti_std_threshold,
        )
        ...
        positions_by_unit = _unit_model_positions(
            prompt_text,
        ...
        input_embeds = (
            self.model.model.get_input_embeddings()(input_ids).detach()
        )
        ...
        if checkpointing:
            if float(getattr(causal_model.config, "attention_dropout", 0.0)):
                raise RuntimeError(
        ...
        try:
            ...
            response_positions = torch.tensor(
                [len(prompt_ids) - 1 + index for index in selected],
                dtype=torch.long,
                device=self.model.device,
            )
            logits = causal_model.lm_head(hidden[0, response_positions]).float()
            log_probs = torch.log_softmax(logits, dim=-1)
            for offset, response_index in enumerate(selected):
...
def _unit_model_positions(
    ...
    for context in contexts:
        prefix = f"[{context.get('chunk_id', '')}] "
        marker = prefix + str(context.get("text", ""))
        marker_start = prompt_text.find(marker, cursor)
        if marker_start < 0:
            raise ValueError(
                f"cannot locate chunk {context.get('chunk_id')} in rendered prompt"
            )
        context_starts[str(context["chunk_id"])] = marker_start + len(prefix)
        cursor = marker_start + len(marker)

    result = []
    for unit in units:
        start = (
            context_starts[str(unit["chunk_id"])]
