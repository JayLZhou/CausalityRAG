...
def fidelity_path(dataset: str, *, run_root: Path, reuse_root: Path) -> Path:
    if dataset == "2wiki":
...
def protocol_metrics(path: Path, *, dataset: str, protocol: str) -> dict:
    ...
    require(violations == 0, f"{dataset}/{protocol}: {violations} protocol violations")
    return {
        "queries": len(rows),
        "answer_flip_rate": sum(bool(row.get("verified_flip")) for row in rows)
        / len(rows),
        ...
        "verified_flips": sum(bool(row.get("verified_flip")) for row in rows),
...
def main() -> None:
    ...
    parser.add_argument("--sentence-root", type=Path, required=True)
    parser.add_argument("--reuse-root", type=Path)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--manifest-out", type=Path, required=True)
    args = parser.parse_args()
    reuse_root = args.reuse_root or args.run_root / "reuse"

    fidelity = {}
    protocols = {}
    for dataset in DATASETS:
        pool_path = args.run_root / "inputs" / dataset / "shared_pool.jsonl"
        ...
        fidelity[dataset] = audit_fidelity(
            fidelity_path(dataset, run_root=args.run_root, reuse_root=reuse_root),
            args.run_root / "out" / dataset / "fidelity" / "seed_audit.json",
            dataset=dataset,
        )
        metrics = {}
    ...
    result = {
        "schema": "causalityrag.figure10_11.v1",
    ...
    manifest = {
        ...
        "paper_render_command": (
            ...
            "--analysis figures/data/reflow_analysis_v1.json "
            "--figure10-11 figures/data/figure10_11_v1.json "
            "--quartz-oracle figures/data/reflow_oracle_quartz_v1.json "
