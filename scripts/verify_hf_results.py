...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional JSONL with id and target_answer. When provided, keep "
            "these clean answers fixed and generate edited variants only."
        ),
    )
    parser.add_argument(
        "--model-path",
    ...
    if args.clean_targets:
        clean_answers_by_id = {
            str(row.get("id", "")): str(row.get("target_answer", ""))
    ...
    with open(args.out, "w", encoding="utf-8") as output:
        for index, identifier in enumerate(evaluations_by_id, 1):
            ...
            if clean_answers_by_id is None:
                clean_response = response_by_signature[()]
                clean_answer = answer_from_response(clean_response)
            else:
                clean_answer = clean_answers_by_id[identifier]
                clean_response = json.dumps(
                    {"answer": clean_answer},
                    ensure_ascii=False,
                )
                response_by_signature[()] = clean_response
            ...
            output.flush()
            print(
                f"[hf-verification] {index}/{len(evaluations_by_id)} "
    ...
    print("[hf-verification summary]", rendered)
    if args.summary_out:
        with open(args.summary_out, "w", encoding="utf-8") as output:
            output.write(rendered + "\n")


def load_evaluation_methods(
    paths: list[str],
