...
def build_payload(root: Path, datasets: list[str]) -> dict[str, Any]:
    ...
    return {
        "schema": "causalityrag.table3_mean_edited_tokens.v1",
        "table3_datasets": datasets,
        "population": "all 1,000 frozen queries per dataset",
        "token_count_contract": {
            ...
            "invalid_clean_or_no_attempt": "zero",
            "baselines": (
        ...
        "diagnostic_population": (
            "valid-clean-only means are retained for audit and are not plotted"
        ),
        "datasets": rows,
        "unweighted_dataset_macro_mean": statistics.fmean(
...
def render_tex(payload: dict[str, Any]) -> str:
    ...
    return "\n".join((
        ...
        tex_coordinates(rows, "mean_edited_tokens_all_1000") + "}",
        "\\def\\TableThreeValidCleanMeanEditedTokenCoordinates{" +
        tex_coordinates(rows, "mean_edited_tokens_valid_clean") + "}",
        f"\\def\\TableThreeMeanEditedTokenMacro{{{payload['unweighted_dataset_macro_mean']:.3f}}}",
        "",
    ))
