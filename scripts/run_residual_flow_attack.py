"""Verify residual contribution-flow cuts with matched token replacements."""
...
def main() -> None:
    ...
    with open(args.out, "w", encoding="utf-8") as output:
        for gate_row in gate_rows:
            ...
            output.flush()
            print(
                f"[residual-flow attack] index={row['index']} "
    ...
    print("[residual-flow attack summary]", rendered)
    if args.summary_out:
...
def threshold_candidate(
    ...
    eligible = [
        ...
        if 0 < int(candidate.get("n_selected", 0)) <= max_tokens
