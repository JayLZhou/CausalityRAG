"""Find unrestricted dense token sets, replace them, and verify reader flips."""
...
def main() -> None:
    ...
    with open(args.out, "w", encoding="utf-8") as output:
        for index, (record, graph_row) in enumerate(zip(records, graphs), 1):
            ...
            candidate_ids = set(candidate["selected_ids"])
            ...
            output.flush()
            print(
                f"[supermodular-ratio] {index}/{len(records)} "
                f"rounds=1 selected={row['n_selected']} "
                f"flip={changed} seconds={row['elapsed_seconds']}",
                flush=True,
            )

    flips = [row for row in rows if row["answer_changed"]]
    with_attempts = [row for row in rows if row["rounds_tried"]]
    print("[supermodular-ratio summary]", json.dumps({
        "records": len(rows),
        "flips": len(flips),
        ...
        "avg_selected_on_flip": round(
            sum(row["n_selected"] for row in flips) / max(1, len(flips)),
            3,
        ),
        "out": args.out,
    }, ensure_ascii=False))
